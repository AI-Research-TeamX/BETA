"""
Phase 1, Step 2: Probing Classifier Training (GPU-accelerated)
===============================================================
Train logistic regression probes on extracted hidden states for 6 game-theoretic
concept labels. Uses PyTorch on GPU for fast training across all layers in parallel.

Usage:
    python train_probes.py --model_name Qwen2.5-1.5B-Instruct
    python train_probes.py --model_name Qwen2.5-3B-Instruct --pooling mean
"""
...
def train_probe_gpu(X_tensor, y_tensor, n_classes, weight_decay=1e-3, lr=0.1, max_epochs=500):
...
def predict_gpu(model, X_tensor):
    model.eval()
    with torch.no_grad():
...
def train_probe_for_label(X_all, y_all, label_name):
    le = LabelEncoder()
    ...
    actual_classes = [classes[c] for c in le2.classes_] if hasattr(le2, 'classes_') else classes
    ...
    for wd in REG_VALUES:
        model = train_probe_gpu(X_train_t, y_train_t, n_classes, weight_decay=wd)
        val_pred = predict_gpu(model, X_val_t)
        val_acc = accuracy_score(y_val, val_pred)
        if val_acc > best_acc:
            best_acc = val_acc
            best_wd = wd

    final_model = train_probe_gpu(X_trainval_t, y_trainval_t, n_classes, weight_decay=best_wd)

    y_pred = predict_gpu(final_model, X_test_t)
...
def run(model_name, pooling="last"):
    ...
    for layer_idx in range(num_layers):
        ...
        for label_name in LABELS:
            ...
            result = train_probe_for_label(X, y, label_name)
            if result:
                results[layer_key][label_name] = result

        accs = {k: v["test_accuracy"] for k, v in results[layer_key].items()}
        elapsed = time.time() - t0
        print(f"  Layer {layer_idx:2d}: {accs} ({elapsed:.1f}s)")
