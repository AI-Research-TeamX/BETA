#!/bin/bash
# Parameterized Nash-GRPO run. All conditions share the SAME rule reward and data;
# only the advantage estimator (and NASH_BETA) differ -> val reward IS comparable.
# Env: SEED, ADV(grpo|nash_grpo|rank_grpo), NASH_BETA, GPUS, N_GPUS, RAY_TMP, EXP_NAME, OUT_DIR, STEPS_EPOCHS(2)
set -x
set -u
SEED=${SEED:-0}; ADV=${ADV:-grpo}; NASH_BETA=${NASH_BETA:-4.0}
GPUS=${GPUS:-0,1,2,3,4,5,6,7}; N_GPUS=${N_GPUS:-8}; RAY_TMP=${RAY_TMP:-/tmp/ray_default}
STEPS_EPOCHS=${STEPS_EPOCHS:-2}; EXP_NAME=${EXP_NAME:?}; OUT_DIR=${OUT_DIR:?}
...
REWARD_FN="$W/verl_scripts/gamesolve_reward_verl.py"
TRAIN_DATA="$W/data/verl_gamesolve/train.parquet"
VAL_DATA="$W/data/verl_gamesolve/val.parquet"
...
python3 -m verl.trainer.main_ppo \
    ...
    actor_rollout_ref.actor.ppo_mini_batch_size=16 actor_rollout_ref.actor.ppo_micro_batch_size_per_gpu=8 \
    ...
    trainer.project_name=nashgrpo trainer.experiment_name="$EXP_NAME" \
    ...
    trainer.val_before_train=True trainer.default_local_dir="$OUT_DIR" \
