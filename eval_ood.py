"""
Evaluate a checkpoint on OOD GameSolve benchmark.
Reports metrics by OOD category and task type.
...
def format_prompt(sample):
    desc = sample["descriptions"].get("abstract",
           next(iter(sample["descriptions"].values())))
...
def main():
    ...
    parser.add_argument("--bench_path", default="gamesolve_ood_bench.jsonl")
    ...
    print(f"Evaluating {len(samples)} OOD samples...")
    ...
    for i, sample in enumerate(samples):
        ...
        ood_cat = sample.get("metadata", {}).get("ood_category", "unknown")
    ...
    out_path = Path(args.output_dir) / "ood_eval_summary.json"
    with open(out_path, "w") as f:
        json.dump(summary, f, indent=2)

    details_path = Path(args.output_dir) / "ood_eval_details.json"
